local markerPosition = { x = 1535.61865, y = -1683.03809, z = 13.54688 }
local vehicleSpawnPosition = { x = 1525.54712, y = -1677.75037, z = 13.38281 }
local destroyMarkerPosition = { x = 1541.13037, y = -1678.88708, z = 13.55195 }
local allowedACLGroup = "pcc"
...
local marker = createMarker(markerPosition.x, markerPosition.y, markerPosition.z - 1, "cylinder", 1.5, 255, 80, 80, 140)
...
local function hasPanelAccess(player)
    ...
    return isObjectInACLGroup("user." .. getAccountName(account), aclGroup)
end
...
addEventHandler("onMarkerHit", marker, function(hitElement, matchingDimension)
    ...
    if not hasPanelAccess(hitElement) then
        outputChatBox("Voce nao tem permissao para abrir este painel PCC.", hitElement, 255, 80, 80)
        return
    end
...
addEventHandler(eventPrefix .. ":onSelectSkin", root, function(skinId)
    ...
    if not skins[skinId] then
        outputChatBox("Roupa invalida.", client, 255, 80, 80)
        return
    end
